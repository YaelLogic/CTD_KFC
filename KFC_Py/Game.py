...
class Game:
    def __init__(self, pieces: List[Piece], board: Board, score_observer_1=None, score_observer_2=None, display=None):
        ...
        self.pieces = pieces
        self.board = board
        # Score observers for each player (A=White, B=Black)
        self.score_observer_1 = score_observer_1 or ScoreObserver()
        self.score_observer_2 = score_observer_2 or ScoreObserver()
        # היסטוריית מהלכים
        self.move_history_1 = MoveHistoryObserver()  # לבן
        self.move_history_2 = MoveHistoryObserver()  # שחור
        ...
        for cell, plist in occupied.items():
            ...
            for p in plist:
                ...
                if p.state.can_be_captured():
                    # Determine which player is the winner (A/White or B/Black)
                    # Assuming winner.id[1] is 'W' for player 1, 'B' for player 2
                    if winner.id[1] == 'W':
                        self.score_observer_1.add_piece_points(p.id[0])
                    elif winner.id[1] == 'B':
                        self.score_observer_2.add_piece_points(p.id[0])
